...
SUSLIK_JAR   = 'suslik/target/scala-2.12/suslik.jar'                      # Path to suslik.jar
TIMEOUT      = '-t=120000'                                                # Timeout option for suslik
TEST_DIR     = 'suslik/src/test/resources/synthesis/paper-benchmarks/'    # Root directory for the tests   
...
def run_benchmark(file):
  ...
  with open(RESULTS, "a") as outfile:
    print ('Running', file)
    commands = [JAVA8, '-jar', SUSLIK_JAR, file, TIMEOUT]
    print(' '.join(commands))
    call(commands, stdout=outfile)
    read_csv()
