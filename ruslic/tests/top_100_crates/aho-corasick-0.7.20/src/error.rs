...
impl Error {
    ...
    pub(crate) fn state_id_overflow(max: usize) -> Error {      let kind = crate::error::ErrorKind::StateIDOverflow { max: max as usize };
      crate::error::Error { kind }
    }

    pub(crate) fn premultiply_overflow(        max: usize,
        requested_max: usize,
    ) -> Error {
